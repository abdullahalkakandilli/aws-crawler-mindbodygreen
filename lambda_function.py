...
Get inital datetime from cralwtext.txt - aws 3 --> last_execute

Get last_feed datetime from mindbodygreen rss --> last_feed
...
def lambda_handler(event, context):
    ...
    feed = feedparser.parse('https://www.mindbodygreen.com/rss/feed.xml')        
    key2 = 'crawltext.txt'
    fileObj3 = s3.get_object(Bucket=bucketname, Key="crawltext.txt")
    with open('/tmp/crawltext.txt', 'w') as f:
        f.write(feed.entries[0].published)
       
    bucket = s3v2.Bucket(bucketname)
    bucket.upload_file('/tmp/crawltext.txt',key2)
    
    #--------------------------------------------------------------------#
    
    key = 'mycrawrecord.csv'
